...
public class JdtOrganizeImportsOperation implements IWorkspaceRunnable {
  ...
  private static class TypeReferenceProcessor {
    ...
    private static class UnresolvedTypeData {
      ...
      public UnresolvedTypeData(SimpleName ref) {
        this.ref = ref;
        this.typeKinds = OrganizeImportsHelper.getPossibleTypeKinds(ref, true);
    ...
    private boolean needsImport(ITypeBinding typeBinding, SimpleName ref) {
      ...
      ITypeBinding currTypeBinding = OrganizeImportsHelper.getBindingOfParentType(ref);
      if (currTypeBinding == null) {
        if (OrganizeImportsHelper.getParent(ref, ASTNode.PACKAGE_DECLARATION) != null) {
      ...
      if (typeBinding.isMember()) {
        if (fAnalyzer.isDeclaredInScope(typeBinding, ref, ScopeAnalyzer.TYPES | ScopeAnalyzer.CHECK_VISIBILITY)) return false;
      }
      return true;
    }
    ...
    public void add(SimpleName ref) {
      ...
      if (binding != null) {
        ...
        if (fDoIgnoreLowerCaseNames && typeName.length() > 0) {
          ...
          if (OrganizeImportsHelper.isLowerCase(ch) && Character.isLetter(ch)) {
            return;
          }
        }
      }
    ...
    public boolean process(IProgressMonitor monitor) throws JavaModelException {
      try {
        ...
        boolean is50OrHigher = OrganizeImportsHelper.is50OrHigher(project);
    ...
    private boolean isOfKind(TypeNameMatch curr, int typeKinds, boolean is50OrHigher) {
      ...
      if (Flags.isAnnotation(flags)) {
        return is50OrHigher && (typeKinds & OrganizeImportsHelper.ANNOTATIONS) != 0;
      }
      if (Flags.isEnum(flags)) {
        return is50OrHigher && (typeKinds & OrganizeImportsHelper.ENUMS) != 0;
      }
      if (Flags.isInterface(flags)) {
        return (typeKinds & OrganizeImportsHelper.INTERFACES) != 0;
      }
      return (typeKinds & OrganizeImportsHelper.CLASSES) != 0;
    }
  ...
  public JdtOrganizeImportsOperation(ICompilationUnit cu, CompilationUnit astRoot, boolean ignoreLowerCaseNames, boolean save, boolean allowSyntaxErrors, IChooseImportQuery chooseImportQuery) {
    fCompilationUnit = cu;
    fASTRoot = astRoot;
...
//      monitor.beginTask(Messages.format(CodeGenerationMessages.OrganizeImportsOperation_description, BasicElementLabels.getFileName(fCompilationUnit)), 10);
      ...
      OrganizeImportsHelper.applyEdit(fCompilationUnit, edit, fDoSave, new SubProgressMonitor(monitor, 1));
    }
    finally {
      monitor.done();
    }
...
//      }
      ...
      ImportRewrite importsRewrite = OrganizeImportsHelper.createImportRewrite(astRoot, false);
      ...
      if (!collectReferences(astRoot, typeReferences, staticReferences, oldSingleImports, oldDemandImports)) return null;
  ...
  private boolean collectReferences(CompilationUnit astRoot, List<SimpleName> typeReferences, List<SimpleName> staticReferences, Set<String> oldSingleImports, Set<String> oldDemandImports) {
    ...
    for (int i = 0; i < imports.size(); i++) {
      ...
      String id = OrganizeImportsHelper.getFullName(curr.getName());
      if (curr.isOnDemand()) {
